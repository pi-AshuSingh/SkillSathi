import dotenv from 'dotenv';
import path from 'path';
import fs from 'fs';
import { mkdir } from 'fs/promises';
import connectDB from '../db.js';
import { Job } from '../models/job.model.js';
import { Company } from '../models/company.model.js';
import { geocodeWithGoogle } from '../utils/geocode.js';

// Load backend/.env explicitly (script may be run from repo root)
const __dirname = path.dirname(new URL(import.meta.url).pathname);
dotenv.config({ path: path.join(__dirname, '..', '.env') });

const argv = process.argv.slice(2);
const dryRun = argv.includes('--dry-run');
const batchArg = argv.find(a => a.startsWith('--batch='));
const pauseArg = argv.find(a => a.startsWith('--pause='));
const limitArg = argv.find(a => a.startsWith('--limit='));
const batchSize = batchArg ? parseInt(batchArg.split('=')[1], 10) : 50;
const pauseMs = pauseArg ? parseInt(pauseArg.split('=')[1], 10) : 500; // ms between geocode calls
const globalLimit = limitArg ? parseInt(limitArg.split('=')[1], 10) : null; // overall max docs to process

const sleep = (ms) => new Promise(r => setTimeout(r, ms));

// prepare logging
const logsDir = path.join(__dirname, 'logs');
await mkdir(logsDir, { recursive: true });
const runTimestamp = new Date().toISOString().replace(/[:.]/g, '-');
const jsonlPath = path.join(logsDir, `backfill_${runTimestamp}.jsonl`);
const csvPath = path.join(logsDir, `backfill_${runTimestamp}.csv`);
const jsonlStream = fs.createWriteStream(jsonlPath, { flags: 'a' });
const csvStream = fs.createWriteStream(csvPath, { flags: 'a' });

// write CSV header
csvStream.write('timestamp,collection,id,location,status,lat,lng,provider,error\n');

function writeLog(entry){
  try{
    jsonlStream.write(JSON.stringify(entry) + '\n');
    // CSV: escape quotes and commas by wrapping in double quotes and doubling inner quotes
    const esc = (s) => {
      if (s === null || s === undefined) return '';
      return `"${String(s).replace(/"/g, '""')}"`;
    }
    const csvRow = [entry.timestamp, entry.collection, entry.id, entry.location || '', entry.status || '', entry.lat || '', entry.lng || '', entry.provider || '', entry.error || ''].map(esc).join(',') + '\n';
    csvStream.write(csvRow);
  } catch (e){
    console.warn('Failed to write log entry', e.message);
  }
}

process.on('exit', () => {
  try{ jsonlStream.end(); csvStream.end(); } catch(e){}
});

async function processCollection(Model, name){
  console.log(`\nProcessing ${name}...`);

  const filter = { $or: [ { locationCoords: { $exists: false } }, { geoLocation: { $exists: false } } ] };
  const total = await Model.countDocuments(filter);
  console.log(`${total} ${name} documents missing geo fields`);
  if (!total) return;

  let processed = 0;
  let page = 0;

  while (processed < total){
    const docs = await Model.find(filter).skip(page * batchSize).limit(batchSize).lean();
    if (!docs.length) break;

    for (const doc of docs){
      const id = doc._id.toString();
      const location = doc.location;

      if (globalLimit && processed >= globalLimit){
        console.log(`Reached global limit of ${globalLimit} for ${name}, stopping.`);
        return;
      }

      if (!location || typeof location !== 'string'){
        console.log(`${name} ${id} has no textual location, skipping`);
        writeLog({ timestamp: new Date().toISOString(), collection: name, id, location: doc.location || '', status: 'skipped-no-location' });
        processed++;
        continue;
      }

      try{
        const geo = await geocodeWithGoogle(location);
        if (!geo){
          console.log(`${name} ${id} -> geocode returned no result for "${location}"`);
          writeLog({ timestamp: new Date().toISOString(), collection: name, id, location, status: 'no-result' });
        } else {
          console.log(`${name} ${id} -> ${geo.lat},${geo.lng} (provider: ${geo.provider})`);
          if (!dryRun){
            await Model.findByIdAndUpdate(id, {
              locationCoords: [geo.lat, geo.lng],
              geoLocation: { type: 'Point', coordinates: [geo.lng, geo.lat] },
              geocodedAt: new Date(),
              geocodeProvider: geo.provider,
              geocodeRaw: geo.raw
            });
            console.log(`  -> updated ${name} ${id}`);
            writeLog({ timestamp: new Date().toISOString(), collection: name, id, location, status: 'updated', lat: geo.lat, lng: geo.lng, provider: geo.provider });
          } else {
            console.log(`  -> dry-run: would update ${name} ${id}`);
            writeLog({ timestamp: new Date().toISOString(), collection: name, id, location, status: 'dry-run-would-update', lat: geo.lat, lng: geo.lng, provider: geo.provider });
          }
        }
      } catch (err){
        console.error(`${name} ${id} -> geocode error:`, err.message);
        writeLog({ timestamp: new Date().toISOString(), collection: name, id, location, status: 'error', error: err.message });
      }

      processed++;
      // pause between requests to respect rate limits
      await sleep(pauseMs);
    }

    page++;
  }
}

async function main(){
  if (!process.env.MONGO_URI) {
    console.error('MONGO_URI is not set in env. Aborting.');
    process.exit(1);
  }
  if (!process.env.GOOGLE_MAPS_KEY){
    console.warn('GOOGLE_MAPS_KEY not set; geocoding will likely fail. You can run with --dry-run for testing.');
  }

  await connectDB();

  try{
    await processCollection(Job, 'Job');
    await processCollection(Company, 'Company');
    console.log('\nBackfill complete.');
  } catch (err){
    console.error('Migration failed:', err);
  } finally {
    process.exit(0);
  }
}

main();
